...
async function loadFolders(section){
  ...
  const folderCards = (data||[]).map(f=>`<article class="folder-card" data-folder="${f.id}">
    <div class="folder-icon"><svg viewBox="0 0 24 24"><path d="M3 5a2 2 0 0 1 2-2h4.5l2 2H19a2 2 0 0 1 2 2v1H3V5z" fill="#FFD873"/><path d="M3 8h18a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8z" fill="#F5A623"/></svg></div>
...
if($("globalSearch")) $("globalSearch").addEventListener("input",async e=>{
  ...
  out.innerHTML=data.map(f=>`<article class="folder-card search-hit" data-search-section="${f.section}" data-search-id="${f.id}"><div class="folder-icon"><svg viewBox="0 0 24 24"><path d="M3 5a2 2 0 0 1 2-2h4.5l2 2H19a2 2 0 0 1 2 2v1H3V5z" fill="#FFD873"/><path d="M3 8h18a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8z" fill="#F5A623"/></svg></div><div class="folder-info"><h3>${escapeHtml(f.name)}</h3><p>${escapeHtml(f.section)} · ${escapeHtml(f.description||"")}</p></div></article>`).join("");
