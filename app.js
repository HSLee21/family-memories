console.log("APP.JS family-memories-v165 loaded");
...
async function loadFolders(section){
  ...
  const folderCards = (data||[]).map(f=>`<article class="folder-card" data-folder="${f.id}">
    <div class="folder-icon"><img src="assets/images/folder-icon.jpg" alt=""></div>
...
async function loadFolderItems(type,folderIdOrIds,target,toolbarUploadBtn,selectBtn,browser){
...
// can be restored across re-renders (e.g. after deleting an item)
// instead of silently dropping back to the plain view each time.
const folderModeState = {};
function setFolderMode(browser,type,folderIdOrIds,mode){
...
function renderEventCard(){
  ...
  if(!upcomingEventsCache.length){
    rowHtml = `
      <div class="event-card-row" id="eventCardRow">
        <span class="event-card-emoji">🎉</span>
    ...
    rowHtml = `
      <div class="event-card-row" id="eventCardRow">
        <span class="event-card-emoji">${emoji}</span>
        <div class="event-card-info">
          <h3>${escapeHtml(ev.title)}</h3>
          <p><span class="event-card-date">${formatEventDate(ev._next)}</span><span class="event-card-dot">•</span><span class="event-card-days">${cd.big}${cd.small?" "+cd.small:""}</span></p>
        </div>
      </div>`;
  }
...
if($("globalSearch")) $("globalSearch").addEventListener("input",async e=>{
  ...
  out.innerHTML=data.map(f=>`<article class="folder-card search-hit" data-search-section="${f.section}" data-search-id="${f.id}"><div class="folder-icon"><img src="assets/images/folder-icon.jpg" alt=""></div><div class="folder-info"><h3>${escapeHtml(f.name)}</h3><p>${escapeHtml(f.section)} · ${escapeHtml(f.description||"")}</p></div></article>`).join("");
...
async function openVideoGallery(types,label,opts){
  ...
  function playVideoAt(index){
    ...
    el.muted=true; // start muted so autoplay is never blocked by the browser (spec-guaranteed on all browsers/iOS)
    el.src=v.signedUrl;
    el.load();
